...
Rarefaction analysis was made with 50, 60 and 100 papers to evaluate the sampling effort
```{r rarefaction_curve, scho=TRUE}
data_vector <- data$Total
num_iterations <- 30

rarefaction_df <- tibble(rarefaction_level = seq(1, length(data_vector), by = 1)) %>%
  mutate(values = map(rarefaction_level, ~ replicate(num_iterations, {
    sampled_values <- sample(data_vector, .x)
    mean(sampled_values)
  }))) %>%
  mutate(mean_value = map_dbl(values, ~ mean(.x)),
         sd_value = map_dbl(values, ~ sd(.x)),
     se_value = sd_value / sqrt(num_iterations)) %>%
  mutate(threshold=ifelse(se_value < 0.001, "Low", "High"))

ggplot(rarefaction_df, aes(x = rarefaction_level, y = mean_value, color= threshold)) +
  geom_point() +
  geom_errorbar(aes(ymin = mean_value - sd_value, ymax = mean_value + sd_value), width = 0.2) +
  labs(x = "Rarefaction Level", y = "Average Rarefied Mean Value") +
  theme_minimal()
```
...
Distribution of total scores of the 100 papers analysed
...
The 100 papers analysed distributed per year
...
### Total scores of each of the 100 papers divided by the 3 domains
